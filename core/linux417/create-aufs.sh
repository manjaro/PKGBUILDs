...
rm -rf ../tmp/linux-4.17
mkdir -p ../tmp/linux-4.17
cp -a fs ../tmp/linux-4.17
cp -a include ../tmp/linux-4.17
cp -a Documentation ../tmp/linux-4.17
rm ../tmp/linux-4.17/include/uapi/linux/Kbuild
cd ../tmp
diff -Naur null linux-4.17  | filterdiff | \
sed -e 's|null\(/include/uapi/linux/Kbuild\)|linux-4.17-old\1|;s|^--- null.*|--- /dev/null|;\|linux-4.17/include/uapi/linux/Kbuild|,${\|@@ -0,0 +1 @@|,$d}' | \
bzip2 > aufs$(sed -ne 's|#define.*AUFS_VERSION.*"\(.*\)"|\1|p'  linux-4.17/include/uapi/linux/aufs_type.h).patch.bz2
mv *.bz2 $OLDPWD
cd $OLDPWD
mv *patch* ..
cd ..
