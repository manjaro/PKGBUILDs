...
rm -rvf ../tmp/linux-5.1
mkdir -p ../tmp/linux-5.1
cp -a fs ../tmp/linux-5.1
cp -a include ../tmp/linux-5.1
cp -a Documentation ../tmp/linux-5.1
rm -v ../tmp/linux-5.1/include/uapi/linux/Kbuild
cd ../tmp
diff -Naur null linux-5.1  | filterdiff | \
sed -e 's|null\(/include/uapi/linux/Kbuild\)|linux-5.1-old\1|;s|^--- null.*|--- /dev/null|;\|linux-5.1/include/uapi/linux/Kbuild|,${\|@@ -0,0 +1 @@|,$d}' \
> aufs$(sed -ne 's|#define.*AUFS_VERSION.*"\(.*\)"|\1|p'  linux-5.1/include/uapi/linux/aufs_type.h).patch
mv *.patch $OLDPWD
cd $OLDPWD
mv *.patch ..
cd ..
