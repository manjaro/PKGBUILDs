...
cd aufs4-standalone
git branch -r
git checkout origin/aufs4.14.73+
...
rm -rf ../tmp/linux-4.14
mkdir -p ../tmp/linux-4.14
cp -a fs ../tmp/linux-4.14
cp -a include ../tmp/linux-4.14
cp -a Documentation ../tmp/linux-4.14
rm ../tmp/linux-4.14/include/uapi/linux/Kbuild
cd ../tmp
diff -Naur null linux-4.14  | filterdiff | \
sed -e 's|null\(/include/uapi/linux/Kbuild\)|linux-4.14-old\1|;s|^--- null.*|--- /dev/null|;\|linux-4.14/include/uapi/linux/Kbuild|,${\|@@ -0,0 +1 @@|,$d}' \
> aufs$(sed -ne 's|#define.*AUFS_VERSION.*"\(.*\)"|\1|p'  linux-4.14/include/uapi/linux/aufs_type.h).patch
mv *.patch $OLDPWD
cd $OLDPWD
mv *.patch ..
cd ..
